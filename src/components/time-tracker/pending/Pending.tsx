...
export default function PendingDetailView({ onBack }: PendingDetailViewwProps) {
  ...
  return (
    <div className="flex flex-col h-lvw bg-[#FFFFFF] ">
      ...
      {/* 2. Metrics Grid */}
      <div className="px-3">
        <div className="grid grid-cols-5 gap-4 pt-3 border-t vorder">
          {metrics.map((m, i) => (
            <div
              key={i}
              className="flex items-center gap-3 p-3 bg-[#FDFDFD] border border-gray-100 rounded-lg   cursor-pointer"
            >
              <div
                className={`w-10 h-10 rounded-sm ${m.color} flex items-center justify-center border ${m.border}`}
              >
              ...
              <div className="flex flex-col">
                ...
                <div className="flex items-baseline gap-1 text-xs font-medium leading-5 tracking-tight">
                  <span className="text-[#191F38]">{m.value}</span>
                  {m.total && (
