...
import { Backlog } from "../floating-component/timeline/Backlog";
import { CustomizeView } from "../floating-component/timeline/CustomizeView";
import CreateTask from "../floating-component/timeline/CreateTask";
...
export default function Timeline() {
  const initialTasks = (timelineData as { tasks: TimelineTask[] }).tasks ?? [];

  const [tasks, setTasks] = useState<TimelineTask[]>(() =>
    initialTasks.filter(isValidTask).map((t) => ({ ...t, row: t.row ?? 0 })),
  );

  const [viewMode, setViewMode] = useState<ViewMode>("day");

  const [zoomIndex, setZoomIndex] = useState<number>(ZOOM_LEVELS.indexOf(0));
  const zoomIn = () =>
  ...
  const scrollToTodayActiveView = () => {
    window.dispatchEvent(
      new CustomEvent("timeline:scrollToToday", { detail: { mode: viewMode } }),
    );
  };
  ...
  return (
    <div className="flex flex-col w-full h-screen bg-[#FDFDFD] text-xs font-medium leading-5 tracking-tight overflow-hidden">
      {/* Top toolbar */}
