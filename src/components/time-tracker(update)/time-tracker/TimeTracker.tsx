...
import mockData from "@/data/time-tracker(update)/time-tracker.json";

import Image from "next/image";
import { useMemo, useState } from "react";
import RequestForm from "../my-time-logs/floating/RequestForm";
...
export default function TimeTracker() {
  ...
  const [users, setUsers] = useState<TimeTrackerUser[]>(() =>
    deepClone((mockData as any).timeTracker ?? []),
  );

  const isRowPayable = (row: BreakdownRow) => {
    if (typeof row.payable === "boolean") return row.payable;
    return row.reason === "Prayer Break";
  };

  const toggleExpanded = (userId: string | number) => {
    setUsers((prev) =>
      prev.map((u) =>
        u.id === userId ? { ...u, isExpanded: !u.isExpanded } : u,
      ),
    );
  };

  const toggleBreakdownPayable = (
    ...
    setUsers((prev) =>
      prev.map((u) => {
        ...
        if (!u.breakdown) return u;

        const next = u.breakdown.map((row, idx) => {
          if (idx !== rowIndex) return row;
          const current = isRowPayable(row);
          return { ...row, payable: !current };
        });

        return { ...u, breakdown: next };
      }),
    );
  };

  const visibleUsers = useMemo(() => {
    if (!onlyPayable) return users;

    return users.filter((u) =>
      (u.breakdown ?? []).some((r) => isRowPayable(r)),
    );
  }, [users, onlyPayable]);
    ...
    <div className="space-y-3 leading-5 tracking-[-0.05em] text-[#191F38]">
      ...
      <div className="space-y-3 px-4">
        {visibleUsers.map((user) => {
          const breakdownRows = onlyPayable
            ? (user.breakdown ?? []).filter((r) => isRowPayable(r))
            : (user.breakdown ?? []);
          ...
          return (
            <div
              ...
              className={`border border-[#EBEBEB] ${
                user.isExpanded ? "bg-[#F2F9FE]" : "bg-[#FDFDFD]"
              } rounded-md transition-all ${
                user.isOutOfOffice ? "opacity-50" : "opacity-100"
              }`}
            >
              {/* User Row */}
              ...
              <div className="bg-[#F2F9FE] px-3 ">
                {user.isExpanded &&
                  ...
                  breakdownRows.length > 0 && (
                    <div className="border-t border-[#EBEBEB] py-4">
                      ...
                      <div className="overflow-hidden overflow-y-auto hide-scrollbar rounded-md border border-[#EBEBEB] bg-[#FFFFFF]">
                        <table className="w-full text-left text-[13px] border-collapse">
                          <thead className="bg-white">
                            <tr className="text-[#191F38] border-b  font-medium text-[11px] leading-5 tracking-[-0.05em]">
                          ...
                          <tbody>
                            {breakdownRows.map((row, idx) => {
                              const payable = isRowPayable(row);
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <td className="px-4 py-2 text-center">
                                    <div className="flex justify-center">
                                      {payable ? (
                                        ...
                                        <Button
                                          variant="outline"
                                          className="px-1.75 py-1.5 grayscale opacity-50"
                                          onClick={(e) => {
                                            e.preventDefault();
                                            e.stopPropagation();
                                            toggleBreakdownPayable(
                                  ...
                                  <td className="p-1.5">
                                    <span className="bg-[#FFD1CC] px-6 py-1.25 text-[#191F38] rounded-sm font-medium text-xs">
                                      {row.signedOut}
                                    </span>
                                  </td>

                                  <td className="p-1.5">
                                    <span className="bg-[#DBE9FF] px-6 py-1.25 text-[#191F38] rounded-sm font-medium text-xs">
                                      {row.duration}
                                    </span>
                                  </td>

                                  <td className="p-1.5">
                                    <span className="bg-[#FFE4B6] px-6 py-1.25 text-[#191F38] rounded-sm font-medium text-xs">
                                      {row.break}
                                    </span>
                                  </td>
                                  ...
                                  <td className="p-1.5">
                                    <div
                                      className={`flex items-center gap-1.5 px-1.75 py-0.5 rounded-sm font-bold text-[11px] w-fit
                            ${
