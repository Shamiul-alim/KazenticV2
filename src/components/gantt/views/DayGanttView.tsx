...
export default function DayGanttView(props: {
  tasks: GanttTask[];
  ...
  const buckets = useBuckets("day", range.start, range.end);
  ...
  const baseCell = 72;
  const cellWidth = Math.max(40, Math.round(baseCell * props.zoom));
  ...
  // ✅ match Timeline behavior: scroll to today when event is fired
  useEffect(() => {
    const scrollToToday = (behavior: ScrollBehavior = "smooth") => {
      ...
      const idx = differenceInDays(today, range.start);
    ...
    const handler = (e: Event) => {
      ...
      if (ce.detail?.mode && ce.detail.mode !== "day") return;
